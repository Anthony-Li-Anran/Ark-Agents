 ...
 * Refactored from renderer.js - uses modular components
 */
...
// Import shared constants (without paths - paths are defined locally)
const {
    MODEL_NAME,
    TEXAS_MODEL_NAME,
    MOVING_SPEED,
...
// Model paths - defined locally using __dirname (points to Amiya/src/renderer/)
const MODEL_PATH = path.join(__dirname, '..', '..', '..', 'Models', '002_Amiya');
const TEXAS_MODEL_PATH = path.join(__dirname, '..', '..', '..', 'Models', '102_Texas');

// Note: UI components (chatUI, contextMenu, operatorPanel, notification, dragUtils) 
// are defined inline in this file to maintain exact compatibility with renderer.js
// The modular versions in scripts/renderer/ can be used for future refactoring

// Add CSS styles for chat UI (from chat-ui module)
...
function showContextMenu(x, y) {
    ...
    if (operatorPanelEl && operatorPanelEl.classList.contains('visible')) {
...
function createOperatorPanel() {
    ...
    operatorPanelEl = document.createElement('div');
    operatorPanelEl.className = 'operator-panel';
    operatorPanelEl.innerHTML = `
        <div class="card">
            <div class="content">
                <form class="my-form">
                    <div>
    ...
    const texasCheckbox = operatorPanelEl.querySelector('#operator-texas');
    ...
    const exitButton = operatorPanelEl.querySelector('.operator-exit-btn');
    exitButton.addEventListener('click', () => {
        hideOperatorPanel();
    });

    operatorPanelEl.addEventListener('mousedown', (event) => {
        event.stopPropagation();
    });
    operatorPanelEl.addEventListener('click', (event) => {
        event.stopPropagation();
    });
...
function positionOperatorPanel() {
    if (!operatorPanelEl || !contextMenu) return;

    const menuRect = contextMenu.getBoundingClientRect();
    let left = menuRect.right + 12;
    let top = menuRect.top;

    operatorPanelEl.classList.add('visible');
    const panelRect = operatorPanelEl.getBoundingClientRect();
    ...
    operatorPanelEl.style.left = `${left}px`;
    operatorPanelEl.style.top = `${top}px`;
}
...
function hideOperatorPanel() {
    if (operatorPanelEl) {
        operatorPanelEl.classList.remove('visible');
    }
}
...
function showChatUI(playRelaxAnimation = true, showTakeoverMessage = false) {
    ...
    if (playRelaxAnimation) {
            playAnimation(getDefaultChatIdleAnimation(), true);
    }
    if (animationTimer) {
        clearTimeout(animationTimer);
        animationTimer = null;
...
function scheduleNextAnimation() {
    ...
    animationTimer = setTimeout(() => {
        ...
        const nextAnim = getNextAnimation(currentAnimation || getDefaultIdleAnimation());
...
function playTexasAnimation(name, loop = true) {
    ...
    texasAnimation.state.setAnimation(0, name, loop);
...
function getActiveTexasAnimationMatrix() {
    return isSleepTime() ? TEXAS_SLEEP_ANIMATIONS : TEXAS_DAY_ANIMATIONS;
}

function getDefaultTexasIdleAnimation() {
    return isSleepTime() ? 'Sleep' : (Math.random() < 0.5 ? 'Relax' : 'Relax_Idle');
}

function getNextTexasAnimation(current) {
    const transitions = getActiveTexasAnimationMatrix()[current];
    if (!transitions) {
        return getDefaultTexasIdleAnimation();
    }
...
function scheduleNextTexasAnimation() {
    ...
    if (!texasEnabled || texasIsUserInteracting) {
        return;
    }
    ...
    texasAnimationTimer = setTimeout(() => {
        ...
        const nextAnim = getNextTexasAnimation(texasCurrentAnimation || getDefaultTexasIdleAnimation());

        if (nextAnim === 'Move') {
            const direction = Math.random() < 0.5 ? 'left' : 'right';
            setTexasDirection(direction);
            playTexasAnimation('Move', true);
            startTexasMoving();
        } else {
            stopTexasMoving();
...
function handleTexasEdgeCollision(bounds) {
    const shouldRelax = Math.random() < 0.5;
    if (shouldRelax) {
        playTexasAnimation(getDefaultTexasIdleAnimation(), true);
        stopTexasMoving();
        return;
    }
...
async function showTexasModel() {
    ...
    if (!fs.existsSync(skelPath) || !fs.existsSync(atlasPath) || !fs.existsSync(pngPath)) {
        ...
        const checkbox = operatorPanelEl ? operatorPanelEl.querySelector('#operator-texas') : null;
...
function initDrag() {
    document.addEventListener('mousemove', (e) => {
        ...
        // Get model bounds after position update
        const bounds = targetAnimation.getBounds();
        
        // Strictly prevent model from going outside screen
        // Check left edge
        if (bounds.x < 0) {
            targetAnimation.x -= bounds.x;
        }
        // Check right edge
        if (bounds.x + bounds.width > screenWidth) {
            targetAnimation.x -= (bounds.x + bounds.width - screenWidth);
        }
        // Check top edge
        if (bounds.y < 0) {
            targetAnimation.y -= bounds.y;
        }
        // Check bottom edge
        if (bounds.y + bounds.height > screenHeight) {
            targetAnimation.y -= (bounds.y + bounds.height - screenHeight);
        }
    ...
    document.addEventListener('mouseup', (e) => {
        ...
        if (!isDragging && moveDistance < DRAG_THRESHOLD) {
            ...
            if (chatVisible && activePointerModel === 'amiya') {
                ...
                playTexasAnimation('Interact', false);
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        if (moveDistance >= DRAG_THRESHOLD && !isDragging) {
            ...
            if (activePointerModel === 'texas') {
                ...
                playAnimation(chatVisible ? getDefaultChatIdleAnimation() : getDefaultIdleAnimation(), true);
            }
        }
    });
}
