/**
 * Kaltsit Character
 * Handles Kaltsit character rendering, animation and movement
 */
...
class KaltsitCharacter {
    ...
    async load() {
        ...
        if (!fs.existsSync(skelPath)) {
            console.error(`Kaltsit skeleton file not found: ${skelPath}`);
            return;
        }
        if (!fs.existsSync(atlasPath)) {
            console.error(`Kaltsit atlas file not found: ${atlasPath}`);
            return;
        }
        if (!fs.existsSync(pngPath)) {
            console.error(`Kaltsit texture file not found: ${pngPath}`);
            return;
        }
        ...
        try {
            ...
            console.error(`Error loading Kaltsit model: ${error.message}`);
        }
    }
    ...
    getNextAnimation(current) {
        const transitions = KALTSIT_ANIMATIONS[current];
        if (!transitions) {
    ...
    scheduleNextAnimation(isChatVisible, isUserInteracting) {
        // Auto-animation is disabled - Kaltsit stays in Relax2 only
