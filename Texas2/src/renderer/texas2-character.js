/**
 * Texas2 Character
 * Handles Texas2 character rendering, animation and movement
 */
...
class Texas2Character {
    ...
    async load() {
        ...
        if (!fs.existsSync(skelPath)) {
            console.error(`Texas2 skeleton file not found: ${skelPath}`);
            return;
        }
        if (!fs.existsSync(atlasPath)) {
            console.error(`Texas2 atlas file not found: ${atlasPath}`);
            return;
        }
        if (!fs.existsSync(pngPath)) {
            console.error(`Texas2 texture file not found: ${pngPath}`);
            return;
        }
        ...
        try {
            ...
            console.error(`Error loading Texas2 model: ${error.message}`);
        }
    }
    ...
    getNextAnimation(current) {
        const transitions = TEXAS2_ANIMATIONS[current];
        if (!transitions) {
    ...
    scheduleNextAnimation(isChatVisible, isUserInteracting) {
        // Auto-animation is disabled - Texas2 stays in Relax2 only
